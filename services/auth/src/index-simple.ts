...
authRouter.post("/waitlist/simple", async (req, res) => {
  try {
...
authRouter.post("/wallet/signup", async (req, res) => {
  try {
    const { 
      email, 
      name, 
    ...
    // Generate user ID
    const userId = existingProfile?.userId || crypto.randomBytes(32).toString('hex');
...
authRouter.post("/refresh", async (req, res) => {
  try {
    ...
    const decoded = jwt.verify(refreshToken, APP_REFRESH_SECRET) as any;
